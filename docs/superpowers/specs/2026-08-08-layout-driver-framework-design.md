...
- Playwright drives headless Chrome against the layout-server's URL (default `https://localhost:8443/`), after polling `/healthz`. Headless, not headed: nothing here needs a real on-screen window (app audio is a plain `<audio>` element, no DRM), and capturing a *visible* window via CDP screenshots at 30fps was observed live to hammer the compositor enough to cause visible on-screen flashing — confirmed by isolating the same headed launch with the capture loop removed entirely (no flashing without it). Headless has no on-screen presentation to disrupt.
- Capture backend is config-selectable (`config/broadcaster.yaml: capture_backend: cdp | sck`), default `cdp`:
  - `cdp`: Chrome DevTools Protocol screencast (`Page.startScreencast`, JPEG, `maxWidth`/`maxHeight` set to the configured resolution), push-based — Chrome delivers a frame only on real compositor damage, acked via `Page.screencastFrameAck` (a strong reference is kept on each ack's task until it completes; `asyncio.create_task()` alone only holds a *weak* reference, so with nothing else referencing it the task can be GC'd before the ack is sent — CDP's screencast is flow-controlled and withholds the next frame until the current one is acked, so a dropped ack eventually stalls the whole pipeline). This is the third capture mechanism tried here, in order: (1) originally this same screencast mechanism, under a *headed* kiosk window — dropped because a headed window's real OS window can be capped smaller than the configured resolution by the physical display, and `buildRoot()`'s `transform: scale()` (sized to fit whatever the real window/display happens to be) then produces a captured frame whose scaled-down layout, stretched back up to the configured resolution, comes out with screens misplaced and wrongly sized (observed live: a 3456×2234 laptop screen against a 3840×2160 canvas); (2) briefly, polling the layout-server's own `POST /api/screenshot` over HTTP against a *headless* browser — dropped because it conflated two unrelated concerns (the continuous NDI feed and flux-gallery's occasional history-snapshot use of that same endpoint) and added an unnecessary WebSocket/HTTP round trip; (3) briefly, screenshotting `#layout-driver-root` directly via `page.screenshot()` clipped to its `bounding_box()`, paced to `config.fps` — correct and avoided (2)'s round trip, but an extended live run showed capture latency climbing steadily over several minutes of sustained 30fps polling (30ms → 200ms+, confirmed independent of memory growth, the flux-gallery worker's activity, and sender-thread decode/send cost, which stayed flat) with no corresponding growth in any process's RSS; `Page.captureScreenshot` is built for occasional one-off snapshots, not sustained continuous capture, and something about repeated calls at video rates accumulates cost over time. Landing back on screencast, now under headless, sidesteps (1)'s original blocker entirely: headless has no physical window to be smaller than anything, so the whole class of aspect-ratio bug that caused (1) cannot occur, and a live extended run (both apps, several minutes each) confirmed stable, flat decode/send latency with no degradation.
