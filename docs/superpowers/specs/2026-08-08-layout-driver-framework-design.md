...
- Capture backend is config-selectable (`config/broadcaster.yaml: capture_backend: cdp | sck`), default `cdp`:
  - `cdp`: screenshots `#layout-driver-root` directly (`page.screenshot()` clipped to `root`'s `bounding_box()`, re-derived each frame so it stays correct across any resize), paced to `config.fps` via the same deadline-based clock the sender thread uses. `#layout-driver-root`'s CSS width/height are always the canvas's fixed dimensions — `buildRoot()`'s `transform: scale()` (sized to fit whatever the real window/display happens to be) scales both dimensions uniformly, so it never changes the aspect ratio, unlike capturing the whole viewport (which includes whatever letterboxing surrounds `root` and previously caused screens to come out misplaced and wrongly sized on a display whose aspect ratio doesn't match the canvas's, e.g. a 3456×2234 laptop screen against a 3840×2160 canvas). This is the third capture mechanism tried here: originally Chrome DevTools Protocol screencast (`Page.startScreencast`, replaced for the aspect-ratio bug above), then briefly polling the layout-server's own `POST /api/screenshot` over HTTP (replaced because it conflated two unrelated concerns — the continuous NDI capture feed and flux-gallery's occasional history-snapshot endpoint — and added an unnecessary WebSocket/HTTP round trip for something achievable by screenshotting the already-open page directly).
