...
- Capture backend is config-selectable (`config/broadcaster.yaml: capture_backend: cdp | sck`), default `cdp`:
  - `cdp`: polls the layout-server's own `POST /api/screenshot` (the same in-page canvas compositor `enableScreenshotResponder()` uses, driven over the existing `/ws` connection to the kiosk tab) once per second, PNG-decoded via Pillow. No OS permissions, works on any platform Playwright supports. Originally implemented via Chrome DevTools Protocol screencast (`Page.startScreencast`); replaced because that path captures the kiosk window's own CSS-scaled layout (`buildRoot()`'s `transform: scale()` to fit whatever the real display's viewport happens to be), which only matches the canvas's 3840×2160 composite when the physical display's aspect ratio happens to equal the canvas's — on a display that doesn't (e.g. a 3456×2234 laptop screen against the 3840×2160 canvas), captured screens came out misplaced and wrongly sized once stretched to the configured resolution. Polling `/api/screenshot` reuses the one compositor that's independent of window/display size instead of maintaining a second, display-dependent one.
